...
def main() -> None:
    ...
    if args.wandb_name is not None and is_rank_0():
        ...
        # --- ① API キーがあれば online で再ログイン --------------------------
        if "WANDB_API_KEY" in os.environ and os.environ["WANDB_API_KEY"]:
            os.environ["WANDB_MODE"] = "online"                 # 強制 online
            wandb.login(key=os.environ["WANDB_API_KEY"], relogin=True)
        else:
            # API キーが無い時だけ offline を維持
            os.environ.setdefault("WANDB_MODE", "offline")
        # --------------------------------------------------------------------

        # force offline mode to avoid missing API key on headless compute
        #os.environ.setdefault("WANDB_MODE", "offline")
        import datetime
        ...
        # init in offline mode if no API key provided
        wandb.init(mode=os.environ.get("WANDB_MODE"), **wandb_setting)
    ...
    if torch_distributed.is_initialized():
        ...
        print(f"[rank {torch_distributed.get_rank()}] World size: {torch_distributed.get_world_size()}", flush=True) #Print RANK and WORLD SIZE
